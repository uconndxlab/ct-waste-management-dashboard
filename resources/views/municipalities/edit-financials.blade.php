...
@section('content')
    ...
    <form action="{{ route('municipalities.financials.update', ['municipality' => $financialData->municipality]) }}" method="POST">
        ...
        <div class="form-group mb-2">
            ...
            <input type="text" name="time_period" class="form-control" value="{{ old('time_period', $financialData->time_period) }}">
        </div>

        <div class="form-group mb-2">
            <label><strong>Town Population (2022)</strong></label>
            <input type="number" name="population" class="form-control" value="{{ old('population', $financialData->population) }}">
        </div>

        <div class="form-group mb-2">
            <label><strong>Town Size (Square Miles, 2010)</strong></label>
            <input type="text" name="size" class="form-control" value="{{ old('size', $financialData->size) }}">
        </div>

        <div class="form-group mb-2">
            <label><strong>Link to Town Budget</strong></label>
            <input type="url" name="link" class="form-control" value="{{ old('link', $financialData->link) }}">
        </div>

        <div class="form-group mb-2">
            <label><strong>Notes</strong></label>
            <textarea name="notes" class="form-control">{{ old('notes', $financialData->notes) }}</textarea>
        </div>
